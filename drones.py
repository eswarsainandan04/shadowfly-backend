...
def add_drone():
    if request.method == 'POST':
        ...
        # ... (similar validation for max_playload)
        max_playload = None
        if max_playload_str and max_playload_str.strip():
            try: max_playload = float(max_playload_str)
            except ValueError:
...
def update_drone(drone_db_id):
    if request.method == 'POST':
        ...
        drone_id_val = request.form.get('drone_id')
        # ... (get all other form fields like in add_drone)
        drone_name = request.form.get('drone_name')
        model = request.form.get('model')
        drone_type = request.form.get('drone_type')
        weight_str = request.form.get('weight')
        ...
        if weight_str and weight_str.strip():
            try: weight = float(weight_str)
            except ValueError: return jsonify({"message": "Invalid Weight. Must be a number.", "error": True}), 400
        max_playload = None
        if max_playload_str and max_playload_str.strip():
            try: max_playload = float(max_playload_str)
            except ValueError: return jsonify({"message": "Invalid Max Payload. Must be a number.", "error": True}), 400
...
def delete_drone(drone_db_id):
    ...
    if conn:
        try:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM dronesdata WHERE id = %s RETURNING id", (drone_db_id,))
                if cur.fetchone() is None:
                    return jsonify({"message": f"Drone with Sys. ID {drone_db_id} not found.", "error": True}), 404
            conn.commit()
            return jsonify({"message": f"Drone {drone_db_id} deleted successfully!"}), 200
        except psycopg2.Error as e:
            conn.rollback()
