    ...
    try:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS users_data (
                ...
                assigned_warehouse VARCHAR(225),
                assigned_ddt varchar(225),
        ...
        conn.commit()
        logging.info("Tables 'users_data' and 'ddts' ensured to exist.")
...
def add_user():
    ...
    required_fields = ['full_name', 'email', 'username', 'role', 'phone_number', 'assigned_to', 'status']
    ...
    assigned_ddt = data.get('assigned_ddt')
    ...
    try:
        ...
        insert_query = """
        INSERT INTO users_data (full_name, email, username, password, role, phone_number, assigned_to, assigned_warehouse, assigned_ddt, status)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;
        """
        cur.execute(insert_query, (full_name, email, username, hashed_password, role, phone_number, assigned_to, assigned_warehouse, assigned_ddt, status))
...
def update_user(user_id):
    ...
    try:
        ...
        SET full_name = %s, email = %s, username = %s, password = %s, role = %s,
            phone_number = %s, assigned_to = %s, assigned_warehouse = %s, assigned_ddt = %s, status = %s
        WHERE id = %s;
        """
        cur.execute(update_query, (full_name, email, username, hashed_password, role, phone_number, assigned_to, assigned_warehouse, assigned_ddt, status, user_id))
        conn.commit()
...
def get_users():
    ...
    try:
        ...
        cur.execute("SELECT id, full_name, email, username, role, phone_number, assigned_to, assigned_warehouse, assigned_ddt, status FROM users_data ORDER BY id DESC;")
        users = cur.fetchall()
        cur.close()
...
@app.route('/get_ddt_names', methods=['GET'])
def get_ddt_names():
    """Endpoint to retrieve DDT names from the ddts table."""
    conn = None
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT name FROM ddts ORDER BY name;")
        ddt_names = [row[0] for row in cur.fetchall()]
        cur.close()
        logging.info(f"Fetched {len(ddt_names)} DDT names.")
        return jsonify(ddt_names), 200
    except psycopg2.Error as e:
        logging.error(f"Database error during DDT names retrieval: {e}")
        return jsonify({"error": "Failed to retrieve DDT names due to a database error."}), 500
    except Exception as e:
        logging.error(f"An unexpected error occurred during DDT names retrieval: {e}")
        return jsonify({"error": "An internal server error occurred."}), 500
    finally:
        if conn:
            conn.close()

@app.route('/get_warehouse_names', methods=['GET'])
def get_warehouse_names():
    """Endpoint to retrieve warehouse names from the warehouses table."""
    conn = None
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT name FROM warehouses ORDER BY name;")
        warehouse_names = [row[0] for row in cur.fetchall()]
        cur.close()
        logging.info(f"Fetched {len(warehouse_names)} warehouse names.")
        return jsonify(warehouse_names), 200
    except psycopg2.Error as e:
        logging.error(f"Database error during warehouse names retrieval: {e}")
        return jsonify({"error": "Failed to retrieve warehouse names due to a database error."}), 500
    except Exception as e:
        logging.error(f"An unexpected error occurred during warehouse names retrieval: {e}")
        return jsonify({"error": "An internal server error occurred."}), 500
    finally:
        if conn:
            conn.close()
...
# Modified endpoint to get user details by username with conditional assignment display
...
def get_user_details(username):
    ...
    try:
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute("SELECT id, full_name, email, username, role, phone_number, assigned_to, assigned_warehouse, assigned_ddt, status FROM users_data WHERE username = %s;", (username,))
        user = cur.fetchone()
        cur.close()
        ...
        if user:
            ...
            # Add conditional assignment field based on assigned_to value
            if user_dict.get('assigned_to'):
                assigned_to_lower = user_dict['assigned_to'].lower()
                if assigned_to_lower == 'ddt':
                    user_dict['assignment_display'] = {
                        'type': 'DDT',
                        'value': user_dict.get('assigned_ddt', 'Not Assigned')
                    }
                elif assigned_to_lower == 'warehouse':
                    user_dict['assignment_display'] = {
                        'type': 'Warehouse',
                        'value': user_dict.get('assigned_warehouse', 'Not Assigned')
                    }
                else:
                    user_dict['assignment_display'] = {
                        'type': 'Unknown',
...
# New endpoint to check if user exists for password reset
