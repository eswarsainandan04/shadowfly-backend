    ...
    try:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS users_data (
                id SERIAL PRIMARY KEY,
                assigned_ddt CHARACTER VARYING(255),
                assigned_warehouse CHARACTER VARYING(255)
...
# Update the update_ddt function to handle latitude and longitude AND update users_data
...
def update_ddt(ddt_id):
    ...
    try:
        ...
        if updated_ddt_record:
            # Update users_data table if DDT name was changed
            if name is not None and old_ddt_name and old_ddt_name != name:
                app.logger.info(f"[DEBUG][UPDATE DDT ID: {ddt_id}] Updating users_data: '{old_ddt_name}' -> '{name}'")
                cur.execute("""
                    UPDATE users_data 
                    SET assigned_ddt = %s 
                    WHERE assigned_ddt = %s AND assigned_ddt IS NOT NULL
                """, (name, old_ddt_name))
                users_updated = cur.rowcount
                app.logger.info(f"[DEBUG][UPDATE DDT ID: {ddt_id}] Updated {users_updated} users_data records with new DDT name")
...
def delete_ddt(ddt_id):
    ...
    try:
        ...
        cur.execute("""
            UPDATE users_data
            SET assigned_ddt = NULL
            WHERE assigned_ddt = %s
        """, (ddt_name,))
        app.logger.info(f"[DEBUG][DELETE DDT ID: {ddt_id}] Related users_data entries updated (assigned_ddt set to NULL). Rows affected: {cur.rowcount}")
    ...
    try:
        ...
        if updated_warehouse_record:
            if new_name is not None and old_warehouse_name != new_name:
                ...
                # Update users_data table for assigned_warehouse
                app.logger.info(f"[DEBUG][UPDATE WAREHOUSE ID: {warehouse_id}] Updating users_data: '{old_warehouse_name}' -> '{new_name}'")
                cur.execute("""
                    UPDATE users_data 
                    SET assigned_warehouse = %s 
                    WHERE assigned_warehouse = %s AND assigned_warehouse IS NOT NULL
                """, (new_name, old_warehouse_name))
                users_updated = cur.rowcount
                app.logger.info(f"[DEBUG][UPDATE WAREHOUSE ID: {warehouse_id}] Updated {users_updated} users_data records with new warehouse name")
...
def delete_warehouse(warehouse_id):
    ...
    try:
        ...
        cur.execute("""
            ...
            WHERE assigned_warehouse = %s
        """, (warehouse_name_to_delete,))
        app.logger.info(f"[DEBUG][DELETE WAREHOUSE ID: {warehouse_id}] Related users_data entries updated (assigned_warehouse set to NULL). Rows affected: {cur.rowcount}")
